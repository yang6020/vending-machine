module.exports = class VendingMachine {
  ...
  addGood(good) {
    ...
    if (index == -1) {
      ...
      this.goods[index][index + 1] = good;
    }
  }
  replaceGood(slotNumber, good) {
    this.goods[slotNumber - 1][slotNumber] = good;
  }

  refillGood(good) {
    let index = 0;
    for (let i = 0; i < this.goods.length; i++) {
      if (Object.keys(this.goods[i][i + 1]) == good) {
        index = i;
      }
    }
    this.goods[index][index + 1][good].count += 5;
  }
